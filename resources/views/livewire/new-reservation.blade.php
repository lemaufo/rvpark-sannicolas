...
new class extends Component {
    ...
    public function submit()
    {
        ...
        try {
            ...
            $this->reset(['unit_id', 'guest_name', 'guest_phone', 'guest_email', 'nationality', 'license_plate', 'check_in', 'check_out', 'status', 'total_amount']);
...
<div>
    ...
    <button wire:click="$set('showModal', true)"
        class="px-5 py-2.5 bg-emerald-600 text-white hover:bg-emerald-700 dark:bg-emerald-500 dark:hover:bg-emerald-600 rounded-xl font-bold shadow-md hover:shadow-lg transition-all duration-300 flex items-center gap-2 transform hover:-translate-y-0.5">
    ...
    @if($showModal)
        <div
            class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-zinc-900/60 backdrop-blur-sm transition-opacity duration-300">
            <div
                class="bg-white dark:bg-zinc-900 rounded-[2rem] shadow-2xl w-full max-w-lg overflow-hidden border border-zinc-200 dark:border-zinc-800 transform transition-all">
                <div
                    ...
                    <button wire:click="$set('showModal', false)"
                        class="p-2 text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-200 hover:bg-zinc-200 dark:hover:bg-zinc-800 rounded-full transition-colors">
                ...
                <div class="p-6">
                    ...
                    <form wire:submit="submit" class="space-y-6">
                        ...
                        <div class="grid grid-cols-2 gap-5">
                            <div>
                                ...
                                <input type="date" wire:model.live="check_in" min="{{ date('Y-m-d') }}"
                                    class="w-full rounded-xl border-zinc-300 dark:border-zinc-700 dark:bg-transparent dark:text-white shadow-sm focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-colors">
                                @error('check_in') <span
                                    class="text-red-500 text-xs font-semibold mt-1 inline-block">{{ $message }}</span>
                                @enderror
                            </div>
                            <div>
                        ...
                        <div class="grid grid-cols-2 gap-5">
                            <div>
                                <label class="block text-sm font-bold text-zinc-700 dark:text-zinc-300 mb-2">Hora de
                                    Entrada</label>
                                <div
                                ...
                                <select wire:model.live="unit_id"
                                    ...
                                    @foreach($units as $unit)
                                        <option value="{{ $unit->id }}">
                                            ...
                                            ${{ number_format($unit->price_per_day, 2) }}/día @if($unit->status !== 'available')
                        ...
                        <div>
                            <label class="block text-sm font-bold text-zinc-700 dark:text-zinc-300 mb-2">Nombre del
                                Huésped</label>
                        ...
                        <div>
                            <label class="block text-sm font-bold text-zinc-700 dark:text-zinc-300 mb-2">Teléfono
                                (Opcional)</label>
                        ...
                        <div class="grid grid-cols-2 gap-5">
                            <div>
                                ...
                                <select wire:model="nationality" class="w-full rounded-xl border-zinc-300 dark:border-zinc-700 dark:bg-transparent dark:text-white shadow-sm focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-colors py-2.5 px-3">
                            ...
                            <div>
                                ...
                                <input type="text" wire:model="license_plate" placeholder="Ej. ABC-1234" class="w-full rounded-xl border-zinc-300 dark:border-zinc-700 dark:bg-transparent dark:text-white shadow-sm focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-colors py-2.5 px-3">
                        ...
                        @endphp
                        <div
                            class="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800/30 p-5 rounded-2xl flex justify-between items-center mt-6">
                            <div>
                                <span
